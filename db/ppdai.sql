...
CREATE TABLE borrow_detail_page(
    ...
    data_type VARCHAR(20) NOT NULL COMMENT '数据类型：黑名单、收款中、已还清',
 ...
 * 投标记录表（收款中、已还清）
 */
DROP TABLE IF EXISTS paybacklend;
CREATE TABLE paybacklend (
    id INT(12) PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    tender_finish_date VARCHAR(20) NOT NULL COMMENT '投标完成时间',
    paid_amount DOUBLE(12,2) NOT NULL COMMENT '已还清金额',
    code VARCHAR(20) NOT NULL COMMENT '列表编号',
    borrower VARCHAR(20) NOT NULL COMMENT '借款人',
    tender_amount DOUBLE(12,2) NOT NULL COMMENT '实际投标金额',
    data_type VARCHAR(20) NOT NULL COMMENT '数据类型：收款中、已还清',
    UNIQUE KEY UK_paybacklend (code)
);
 ...
 * 投标记录明细表（收款中、已还清）
 */
DROP TABLE IF EXISTS paybacklend_details;
CREATE TABLE paybacklend_details (
    id INT(12) PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    code VARCHAR(20) NOT NULL COMMENT '列表编号',
    payment_date VARCHAR(20) NOT NULL COMMENT '还款日',
    yisbx DOUBLE(12,2) NOT NULL COMMENT '已收本息',
    yisbj DOUBLE(12,2) NOT NULL COMMENT '已收本金',
    yislx DOUBLE(12,2) NOT NULL COMMENT '已收利息',
    yingsbx DOUBLE(12,2) NOT NULL COMMENT '应收本息',
    over_time_interest DOUBLE(12,2) NOT NULL COMMENT '逾期利息',
    over_days INT(6) COMMENT '逾期天数',
    `status` VARCHAR(32) COMMENT '状态',
    data_type VARCHAR(20) NOT NULL COMMENT '数据类型：收款中、已还清',
    UNIQUE KEY UK_paybacklend_details (code, payment_date)
);

/*
 * 逾期收回表
 */
DROP TABLE IF EXISTS late_back;
CREATE TABLE late_back (
    id INT(12) PRIMARY KEY AUTO_INCREMENT COMMENT '主键',
    payment_date VARCHAR(20) NOT NULL COMMENT '还款日期',
    code VARCHAR(20) NOT NULL COMMENT '列表编号',
    borrower VARCHAR(20) NOT NULL COMMENT '借款人',
    capital_recovery DOUBLE(12,2) NOT NULL COMMENT '本金收回',
    interest_recovery DOUBLE(12,2) NOT NULL COMMENT '利息收回',
    over_days INT(6) COMMENT '逾期天数',
	need_return_amount DOUBLE(12, 2) COMMENT '剩余待还本息',
    periods INT(6) COMMENT '期数',
    UNIQUE KEY UK_late_back (payment_date, code)
);
