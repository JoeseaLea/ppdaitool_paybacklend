...
public class POIExcelCreater extends AbstractExcelCreater {
	private static final Logger logger = Logger.getLogger(AbstractExcelCreater.class);
	...
	protected void generateDetail(){
		...
		for(int rowIndex=0; rowIndex<dataList.size(); rowIndex++){
			...
			HSSFRow detailRow = sheet.createRow(startRowIndex + rowIndex);
			//detailRow.setHeightInPoints(detailHeight); //行高。不需要这行，可以自动适应行高
			
			for(int col=0; col<columnList.size(); col++){
				ColumnMetaDataVo md = columnList.get(col);
				
				String columnValue = rowData.get(md.getName());
				...
				if(md.getCellType() == 0){ //数值型
					...
					try{
						...
						logger.info("Field value parse to Double error: id=" + rowData.get("FIELD1") + ", field=" + md.getName() + ", value=" + columnValue);
						value = 0;
					}
