...
//		System.out.println(htmlTable.asText());
		...
		for (HtmlElement htmlElement : trRepaymentDetailslist) {
			...
			for (int i = 1; i < trTemp.size(); i++) {
				try {
					...
					repaymentDetailVo.setWithdraw(Double.valueOf(tdTemp.get(1).asText().replaceAll("¥", "").replaceAll(",", "")));
					repaymentDetailVo.setNotWithdraw(Double.valueOf(tdTemp.get(2).asText().replaceAll("¥", "").replaceAll(",", "")));
					repaymentDetailVo.setNotGetBackCapital(Double.valueOf(tdTemp.get(3).asText().replaceAll("¥", "").replaceAll(",", "")));
					repaymentDetailVo.setNotGetInterest(Double.valueOf(tdTemp.get(4).asText().replaceAll("¥", "").replaceAll(",", "")));
					repaymentDetailVo.setOverTimeInterest(Double.valueOf(tdTemp.get(5).asText().split("/")[0].replaceAll("¥", "").replaceAll(",", "")));
	 ...
	 */
	public List<String> getBlacklistdetailURL(HtmlPage htmlPage) {
		List<String> dataList = new ArrayList<String>();
		@SuppressWarnings("rawtypes")
		List divList = htmlPage.getByXPath("//table[@class='receivetab']//tr[@class='inner-data']//div[@class='list_info']");
		if(divList != null && divList.size() > 0){
			for(int i=0; i<divList.size(); i++){
				HtmlDivision div = (HtmlDivision)divList.get(i);
				List<HtmlElement> aList = div.getElementsByTagName("a");
				if(aList != null && aList.size() > 0){
					String href = ((HtmlAnchor)aList.get(0)).getHrefAttribute();
					dataList.add(href);
				}
			}
		}
