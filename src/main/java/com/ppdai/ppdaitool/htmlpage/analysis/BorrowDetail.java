...
public class BorrowDetail {
	private static final Logger logger = Logger.getLogger(BorrowDetail.class);
...
//		this.htmlPage = htmlPage;
		...
		try {
			...
			fullUsername = divNewLendDetailbox.getElementsByAttribute("div", "class", "newLendDetailInfo clearfix").get(0)
					...
					.getElementsByAttribute("span", "class", "username").get(0).asText().trim();
		} catch (Exception e) {
			logger.error("", e);
		}
	}
	 ...
	 */
	public List<BidRecordVo> getBidRecords(String dataType) {
		...
		try {
			...
			for (HtmlElement htmlElement : bidRecordsElement) {
				try {
					...
					bidRecordVo.setDataType(dataType);
...
//				bidRecordVo.setUserName(userName);
					...
					bidRecordVo.setValidBidCapital((int)Double.parseDouble(liElement.get(3).asText().trim().replaceAll("¥", "").replaceAll(",", "")));
	 ...
	 */
	public List<BidDebtRecordVo> getBidDebtRecords(String dataType) {
		List<BidDebtRecordVo> bidDebtRecords = new ArrayList<BidDebtRecordVo>();
		
		try {
			List<HtmlElement> bidRecordsElementList = htmlBody.getElementsByAttribute("div", "id", "bidTableDebt_div");
			if(bidRecordsElementList != null && bidRecordsElementList.size() > 0){
				List<HtmlElement> bidRecordsElement = bidRecordsElementList.get(0)
						.getElementsByAttribute("div", "class", "scroll-area").get(0).getElementsByTagName("ol");
				
				for (HtmlElement htmlElement : bidRecordsElement) {
					try {
						BidDebtRecordVo bidDebtRecordVo = new BidDebtRecordVo();
						bidDebtRecordVo.setDataType(dataType);
...
//					bidDebtRecordVo.setUserName(userName);
	...
	public List<BorrowDetailVo> getBorrowDetails(String dataType) {
		...
		try {
			...
			if (null != divTabContainElement) {
				...
				for (HtmlElement htmlElement : tableElements) {
					try {
						if (htmlElement.asText().contains("借款标的")) {
				...
				for (HtmlElement htmlElement : trElements) {
					try {
						...
						borrowDetailVo.setDataType(dataType);
//					borrowDetailVo.setUsername(userName);
	...
	public List<NeedReturnRecordNext6MonthVo> getNeedReturnRecordNext6MonthVos(String dataType) {
				...
				if(tableArr.length > 1){
					...
					for (TagNode tagNode : tdArr) {
						try {
							...
							needReturnRecordNext6MonthVo.setDataType(dataType);
	...
	public List<OverTimeRecordLast6MonthVo> getOverTimeRecordLast6MonthVos(String dataType) {
					...
					if(tableArr.length > 2){
						...
						for(TagNode tdNode : tdArr){
							try {
								...
								overTimeRecordLast6MonthVo.setDataType(dataType);
